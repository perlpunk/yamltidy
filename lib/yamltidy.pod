...
=item Process all files in a directory

This will process filenames according to the 'files' entry in the config.

    % yamltidy --inplace --verbose --batch .
    [info] Processed 'file1.yaml' (unchanged)
    [info] Processed 'file2.yml' (changed)
    # short options
    % yamltidy -ivb .
...
=head2 GLOBAL OPTIONS
    ...
    --batch -b          Tidy all files. Needs a directory name or "-" for filenames passed via STDIN (flag)
